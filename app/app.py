...
# Initialize session state for play/stop control
...
# Define Play/Stop functionality
if st.session_state.is_running:
    if st.button("Stop", key="stop_button"):
        st.session_state.is_running = False
        st.session_state.data = None  # Free the data from memory
        st.cache_data.clear()  # Clear the cache to free memory
...
if st.session_state.is_running:
        ...
        if st.button("Actualizar"):
            with st.spinner('Actualizando datos...'):
                try:
                    ...
                    st.session_state.data = df  # Update session state with new data
    ...
    columns_to_show = ["formatted_date", "name", "channel_name", "thread_name", "message", "message_link", "has_spoilers", "max_reaction_count", "total_reactions"]
    ...
    st.write("The application is currently stopped. Press 'Play' to load the data.")
    st.cache_data.clear()  # Clear the cache when stopped to free memory
    gc.collect()  # Force garbage collection to clear memory
